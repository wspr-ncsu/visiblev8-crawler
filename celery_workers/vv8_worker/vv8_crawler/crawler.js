...
function main() {
  ...
  program
    ...
    .option(
      '--headless <headless>',
      'Which headless mode to run visiblev8',
      'new'
    )
    .option(
      '--loiter-time <loiter_time>',
      'Amount of time to loiter on a webpage',
      DEFAULT_LOITER_TIME
    )
    .option(
      '--nav-time <nav_time>',
      'Amount of time to wait for a page to load',
      DEFAULT_NAV_TIME
    )
    .allowUnknownOption(true)
    .description(
      'Visit the given URL and store it under the UID, creating a page record and collecting all data'
    )
    .action(async function (input_url, uid, options) {
      let combined_crawler_args =
        default_crawler_args.concat(crawler_args);
      let show_log = false;
      ...
      await fs.promises.rm(user_data_dir, {
        recursive: true,
        force: true,
      });
    });
